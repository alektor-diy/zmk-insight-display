#include <lvgl.h>

#include <zmk/display.h>
#include <zmk/event_manager.h>

#include <zmk_insight_display/events/state_changed.h>
#include <zmk_insight_display/private.h>
#include <zmk_insight_display/state.h>
#include <zmk_insight_display/widgets/output_status.h>

static sys_slist_t widgets = SYS_SLIST_STATIC_INIT(&widgets);

LV_IMG_DECLARE(sym_usb);
LV_IMG_DECLARE(sym_bt);
LV_IMG_DECLARE(sym_ok);
LV_IMG_DECLARE(sym_nok);
LV_IMG_DECLARE(sym_open);
LV_IMG_DECLARE(sym_1);
LV_IMG_DECLARE(sym_2);
LV_IMG_DECLARE(sym_3);
LV_IMG_DECLARE(sym_4);
LV_IMG_DECLARE(sym_5);

static const lv_img_dsc_t *sym_num[] = {&sym_1, &sym_2, &sym_3, &sym_4, &sym_5};

enum output_symbol {
    output_symbol_usb,
    output_symbol_usb_status,
    output_symbol_bt,
    output_symbol_bt_number,
    output_symbol_bt_status,
};

static void set_output_status(struct zmk_insight_display_widget_output_status *widget,
                              struct zmk_insight_display_state state) {
    const bool runtime_ready = zmk_insight_display_runtime_ready();
    const bool profile_valid = (state.flags & ZMK_INSIGHT_DISPLAY_FLAG_PROFILE_VALID) != 0U;
    const bool ble_valid = (state.flags & ZMK_INSIGHT_DISPLAY_FLAG_BLE_VALID) != 0U;
    lv_obj_t *usb = lv_obj_get_child(widget->obj, output_symbol_usb);
    lv_obj_t *usb_status = lv_obj_get_child(widget->obj, output_symbol_usb_status);
    lv_obj_t *bt = lv_obj_get_child(widget->obj, output_symbol_bt);
    lv_obj_t *bt_number = lv_obj_get_child(widget->obj, output_symbol_bt_number);
    lv_obj_t *bt_status = lv_obj_get_child(widget->obj, output_symbol_bt_status);

    if (!runtime_ready) {
        lv_obj_add_flag(widget->obj, LV_OBJ_FLAG_HIDDEN);
        return;
    }

    lv_obj_clear_flag(widget->obj, LV_OBJ_FLAG_HIDDEN);

    if (state.output == ZMK_INSIGHT_DISPLAY_TRANSPORT_USB) {
        lv_img_set_src(usb_status, &sym_ok);
    } else {
        lv_img_set_src(usb_status, &sym_nok);
    }

    if (profile_valid && state.profile_index < (sizeof(sym_num) / sizeof(sym_num[0]))) {
        lv_img_set_src(bt_number, sym_num[state.profile_index]);
    } else {
        lv_img_set_src(bt_number, &sym_nok);
    }

    if (!profile_valid) {
        lv_img_set_src(bt_status, &sym_open);
    } else if (!ble_valid) {
        lv_img_set_src(bt_status, &sym_nok);
    } else if (state.ble_state == ZMK_INSIGHT_DISPLAY_BLE_STATE_CONNECTED) {
        lv_img_set_src(bt_status, &sym_ok);
    } else if (state.ble_state == ZMK_INSIGHT_DISPLAY_BLE_STATE_ADVERTISING) {
        lv_img_set_src(bt_status, &sym_open);
    } else {
        lv_img_set_src(bt_status, &sym_nok);
    }

    lv_obj_move_foreground(usb);
    lv_obj_move_foreground(usb_status);
    lv_obj_move_foreground(bt);
    lv_obj_move_foreground(bt_number);
    lv_obj_move_foreground(bt_status);
}

static void output_status_update_cb(struct zmk_insight_display_state state) {
    struct zmk_insight_display_widget_output_status *widget;

    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) { set_output_status(widget, state); }
}

static struct zmk_insight_display_state output_status_get_state(const zmk_event_t *eh) {
    const struct zmk_insight_display_state_changed *ev = as_zmk_insight_display_state_changed(eh);

    if (ev != NULL) {
        return ev->state;
    }

    return *zmk_insight_display_state_ptr();
}

ZMK_DISPLAY_WIDGET_LISTENER(widget_insight_output_status, struct zmk_insight_display_state,
                            output_status_update_cb, output_status_get_state)
ZMK_SUBSCRIPTION(widget_insight_output_status, zmk_insight_display_state_changed);

int zmk_insight_display_widget_output_status_init(
    struct zmk_insight_display_widget_output_status *widget, lv_obj_t *parent) {
    if (widget == NULL || parent == NULL) {
        return -1;
    }

    widget->obj = lv_obj_create(parent);
    lv_obj_set_size(widget->obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    lv_obj_set_style_bg_opa(widget->obj, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(widget->obj, 0, 0);
    lv_obj_set_style_pad_all(widget->obj, 0, 0);
    lv_obj_clear_flag(widget->obj, LV_OBJ_FLAG_SCROLLABLE);

    lv_obj_t *usb = lv_img_create(widget->obj);
    lv_obj_align(usb, LV_ALIGN_TOP_LEFT, 1, 4);
    lv_img_set_src(usb, &sym_usb);

    lv_obj_t *usb_status = lv_img_create(widget->obj);
    lv_obj_align_to(usb_status, usb, LV_ALIGN_BOTTOM_LEFT, 2, -7);
    lv_img_set_src(usb_status, &sym_nok);

    lv_obj_t *bt = lv_img_create(widget->obj);
    lv_obj_align_to(bt, usb, LV_ALIGN_OUT_RIGHT_TOP, 6, 0);
    lv_img_set_src(bt, &sym_bt);

    lv_obj_t *bt_number = lv_img_create(widget->obj);
    lv_obj_align_to(bt_number, bt, LV_ALIGN_OUT_RIGHT_TOP, 2, 7);
    lv_img_set_src(bt_number, &sym_1);

    lv_obj_t *bt_status = lv_img_create(widget->obj);
    lv_obj_align_to(bt_status, bt, LV_ALIGN_OUT_RIGHT_TOP, 2, 1);
    lv_img_set_src(bt_status, &sym_open);

    lv_obj_add_flag(widget->obj, LV_OBJ_FLAG_HIDDEN);

    sys_slist_append(&widgets, &widget->node);
    widget_insight_output_status_init();
    return 0;
}

lv_obj_t *zmk_insight_display_widget_output_status_obj(
    struct zmk_insight_display_widget_output_status *widget) {
    return widget->obj;
}
